...
HRESULT InitD3D()
{
    ...
    d3dpp.BackBufferWidth        = min( VideoMode.dwDisplayWidth, 1920 );
    d3dpp.BackBufferHeight       = min( VideoMode.dwDisplayHeight, 1080 );
...
void __cdecl main()
{
	...
	void* address = (void*)0x00010000;
	void* mem = VirtualAlloc(address, size, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	jfunc f = (jfunc)mem;
	...
	swprintf_s(buffer, size, L"result jit: %08x \n", r);
	OutputDebugStringW(buffer);
	ZeroMemory(buffer, sizeof(buffer));


	swprintf_s(buffer, size, L"base address: %08x \n", mbi.BaseAddress);
	OutputDebugStringW(buffer);
	ZeroMemory(buffer, sizeof(buffer));

	swprintf_s(buffer, size, L"region size: %08x \n", mbi.RegionSize);
	OutputDebugStringW(buffer);
	ZeroMemory(buffer, sizeof(buffer));

	swprintf_s(buffer, size, L"state: %08x \n", mbi.State);
	OutputDebugStringW(buffer);
	ZeroMemory(buffer, sizeof(buffer));

	swprintf_s(buffer, size, L"protect: %08x \n", mbi.Protect);
	OutputDebugStringW(buffer);
	ZeroMemory(buffer, sizeof(buffer));
